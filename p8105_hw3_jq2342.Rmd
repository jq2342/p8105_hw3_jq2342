...
The `accel_data` dataframe contains `r nrow(accel_data)` observations and r ncol(accel_data). Important variables include`week`, `day_id`, `activity` and `activity_count`.
...
ny_noaa=ny_noaa %>%
  ...
  mutate(tmax=as.numeric(tmax), tmin=as.numeric(tmin), year=as.integer(year), month=as.integer(month), day=as.integer(day))  %>%
  summarise(snow = median(snow, na.rm=TRUE))
```
The most commonly observed values for snowfall is `summarise(ny_noaa,snow = median(snow, na.rm=TRUE))`
...
ggsave("noaa_plot_1.pdf", noaa_plot_1, width = 8, height = 5)
```
...
noaa_plot_2=ny_noaa %>%
  ...
  ggplot(aes(x=year, y=snow,na.rm=TRUE))+
  geom_histogram()+
  labs(y= "Snowfall", x = "Year")

ggsave("noaa_plot_2.pdf", noaa_plot_2, width = 8, height = 5)
```
